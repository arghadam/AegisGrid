...
const PROCESS_ANALYSIS_INTERVAL: Duration =
    Duration::from_secs(30);

const SIGNATURE_CACHE_DURATION: Duration =
    Duration::from_secs(300);

const SIGNATURE_METADATA_CACHE_DURATION: Duration =
    Duration::from_secs(300);

const FILE_HASH_CACHE_DURATION: Duration =
    Duration::from_secs(300);

const FILE_HASH_BUFFER_SIZE: usize =
    64 * 1024;
...
pub struct ProcessSecurityDetail {
    ...
    pub signature_status:
        CodeSignatureStatus,

    pub signature_metadata:
...
pub struct ProcessSecuritySnapshot {
    ...
    pub observed_processes:
        Vec<ProcessRiskAssessment>,

    pub suspicious_processes:
        Vec<SuspiciousProcess>,
}

impl ProcessSecuritySnapshot {
    pub fn suspicious_count(
        &self,
    ) -> usize {
    ...
    pub fn review_recommended_count(
        &self,
    ) -> usize {
        self.suspicious_processes
            .iter()
    ...
    pub fn signature_unsigned_count(
        &self,
    ) -> usize {
        self.suspicious_processes
            .iter()
    ...
    pub fn signature_invalid_count(
        &self,
    ) -> usize {
        self.suspicious_processes
            .iter()
    ...
    pub fn signature_unknown_count(
        &self,
    ) -> usize {
        self.suspicious_processes
            .iter()
    ...
    pub fn process_detail(
        ...
        self.suspicious_processes
            ...
            .map(
                SuspiciousProcess::detail,
            )
    }

    pub fn highest_risk_process_detail(
        &self,
    ) -> Option<ProcessSecurityDetail> {
        self.suspicious_processes
            .first()
    ...
    pub fn review_process_details(
        &self,
    ) -> Vec<ProcessSecurityDetail> {
        self.suspicious_processes
            .iter()
            .map(
                SuspiciousProcess::detail,
...
pub struct ProcessSecurityAnalyzer {
    ...
    signature_cache:
        HashMap<PathBuf, CachedSignature>,

    signature_metadata_cache:
        HashMap<PathBuf, CachedSignatureMetadata>,

    file_hash_cache:
        HashMap<PathBuf, CachedFileHash>,
...
impl ProcessSecurityAnalyzer {
    pub fn new() -> Self {
        ...
        Self {
            ...
            cached_snapshot:
                ProcessSecuritySnapshot::default(),
    ...
    pub fn snapshot(
        ...
        {
            add_unusual_permissions_signal(
                &executable_path,
                &mut signals,
            );
    ...
    fn signature_metadata(
                ...
                )
        {
            if cached
                .checked_at
                .elapsed()
                < SIGNATURE_METADATA_CACHE_DURATION
            {
                return cached
                    .metadata
                    .clone();
    ...
    fn cleanup_signature_metadata_cache(
        &mut self,
    ) {
        self.signature_metadata_cache
            .retain(
...
fn add_unusual_permissions_signal(
    path: &Path,
    signals: &mut Vec<ProcessRiskSignal>,
) {
    if has_unusual_permissions(
        path,
    ) {
        signals.push(
...
fn read_code_signature_metadata(
    ...
    let mut metadata = parse_code_signature_metadata(
...
fn primary_anomaly_reason(
    signals: &[ProcessRiskSignal],
) -> Option<ProcessAnomalyReason> {
    if signals.contains(
        &ProcessRiskSignal::TemporaryDirectory,
    ) {
        return Some(
...
mod tests {
                ...
                SuspiciousProcess {
                    ...
                    signature_metadata:
                        CodeSignatureMetadata {
                            ...
                            authorities:
                                vec![
                                    "Developer ID Application: AegisGrid Test"
                                        .to_owned(),
                                ],
                        },
    ...
    fn summary_counts_are_correct() {
        ...
        assert_eq!(
            snapshot.analyzed_process_count,
            500,
        );
        ...
        assert_eq!(
            snapshot.suspicious_count(),
            2,
        );

        assert_eq!(
            snapshot.review_recommended_count(),
            1,
        );

        assert_eq!(
            snapshot.high_anomaly_count(),
            1,
        );
    ...
    fn signature_counts_are_correct() {
        ...
        assert_eq!(
            snapshot.signature_valid_count(),
            0,
        );

        assert_eq!(
            snapshot.signature_unsigned_count(),
            1,
        );

        assert_eq!(
            snapshot.signature_invalid_count(),
            1,
        );

        assert_eq!(
            snapshot.signature_unknown_count(),
            0,
        );
    }
    ...
    fn file_hash_cache_updates_after_file_change() {
        ...
        .expect(
            "changed test file write failed",
        );
